...
enum class Module {
    DASHBOARD, EHS_HOME, EHS_AUDITS, EHS_METRICS, LEGAL_MATRIX, STPS, EHS_DOCUMENTS, GRC_SECURITY,
...
fun isModuleVisible(module: Module, role: UserRole): Boolean {
    ...
    if (role == UserRole.SEGURIDAD) return module in listOf(Module.DASHBOARD, Module.EHS_HOME, Module.EHS_AUDITS, Module.EHS_METRICS, Module.LEGAL_MATRIX, Module.STPS, Module.EHS_DOCUMENTS, Module.SETTINGS)
                ...
                Div({ style { flex(1); display(DisplayStyle.Flex); flexDirection(FlexDirection.Column); overflowY("auto") } }) {
                    ...
                    Div({ style { padding(32.px) } }) {
                        when (activeModule) {
                            ...
                            Module.EHS_HOME -> EhsHomeModule { activeModule = it }
