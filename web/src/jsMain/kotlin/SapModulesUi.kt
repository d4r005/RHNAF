    ...
    if (isLoading) { P { Text("Cargando...") } } else {
        Table({ style { width(100.percent) } }) {
            ...
            Tbody {
                ...
                EhsPendingEvidenceRows(evidence, colSpan = 9, moduleType = "waste", linkTargets = items.map { it.id to "${it.residuo} · ${it.fecha}" }, client = client, scope = scope, onLinked = { refresh() })
            }
        }
    }
}
    ...
    if (isLoading) { P { Text("Cargando...") } } else {
        Table({ style { width(100.percent) } }) {
            ...
            Tbody {
                ...
                EhsPendingEvidenceRows(evidence, colSpan = 8, moduleType = "health", linkTargets = items.map { it.id to "${it.nombreEmpleado.trim()} (${it.empleadoId}) · ${it.fecha}" }, client = client, scope = scope, onLinked = { refresh() })
            }
        }
    }
}
    ...
    if (isLoading) { P { Text("Cargando...") } } else {
        Table({ style { width(100.percent) } }) {
            ...
            Tbody {
                ...
                EhsPendingEvidenceRows(evidence, colSpan = 6, moduleType = "chemical", linkTargets = items.map { it.id to "${it.nombre} · ${it.fabricante}" }, client = client, scope = scope, onLinked = { refresh() })
            }
        }
    }
}
    ...
    if (isLoading) { P { Text("Cargando...") } } else {
        Table({ style { width(100.percent) } }) {
            ...
            Tbody {
                ...
                EhsPendingEvidenceRows(evidence, colSpan = 12, moduleType = "incident", linkTargets = items.map { it.id to "${it.tipo} · ${it.fecha}" }, client = client, scope = scope, onLinked = { refresh() })
            }
        }
    }
}
    ...
    if (isLoading) { P { Text("Cargando...") } } else {
        Table({ style { width(100.percent) } }) {
            ...
            Tbody {
                ...
                EhsPendingEvidenceRows(evidence, colSpan = 11, moduleType = "permit", linkTargets = items.map { it.id to "${it.tipo} · ${it.solicitante} · ${it.fechaInicio}" }, client = client, scope = scope, onLinked = { refresh() })
            }
        }
    }
}
    ...
    if (isLoading) { P { Text("Cargando...") } } else {
        Table({ style { width(100.percent) } }) {
            ...
            Tbody {
                ...
                EhsPendingEvidenceRows(evidence, colSpan = 8, moduleType = "ppe", linkTargets = items.map { it.id to "${it.empleado} · ${it.tipoEpp} · ${it.fecha}" }, client = client, scope = scope, onLinked = { refresh() })
            }
        }
    }
}
    ...
    if (isLoading) { P { Text("Cargando...") } } else {
        Table({ style { width(100.percent) } }) {
            ...
            Tbody {
                ...
                EhsPendingEvidenceRows(evidence, colSpan = 9, moduleType = "training", linkTargets = items.map { it.id to "${it.tema} · ${it.fecha}" }, client = client, scope = scope, onLinked = { refresh() })
            }
        }
    }
}
    ...
    if (isLoading) { P { Text("Cargando...") } } else {
        Table({ style { width(100.percent) } }) {
            ...
            Tbody {
                ...
                EhsPendingEvidenceRows(evidence, colSpan = 9, moduleType = "drill", linkTargets = items.map { it.id to "${it.tipo} · ${it.fecha}" }, client = client, scope = scope, onLinked = { refresh() })
            }
        }
    }
}
    ...
    if (isLoading) { P { Text("Cargando...") } } else {
        Table({ style { width(100.percent) } }) {
            ...
            Tbody {
                ...
                EhsPendingEvidenceRows(evidence, colSpan = 11, moduleType = "risk", linkTargets = items.map { it.id to "${it.area} · ${it.riesgoIdentificado}" }, client = client, scope = scope, onLinked = { refresh() })
            }
        }
    }
}
...
fun EhsPendingEvidenceRows(
    ...
    colSpan: Int,
    moduleType: String,
    // Pares (id real del registro, etiqueta legible) de las filas ya visibles
    // en esta misma tabla, para elegir por nombre/fecha en vez de adivinar un
    // ID interno que la interfaz nunca muestra.
    linkTargets: List<Pair<Int, String>>,
    client: HttpClient,
    scope: kotlinx.coroutines.CoroutineScope,
    onLinked: () -> Unit
) {
    evidence.filter { it.moduleRecordId == 0 }.forEach { doc ->
        var selectedId by remember(doc.id) { mutableStateOf(0) }
        ...
        Tr({ style { backgroundColor(Color("#fffbeb")) } }) {
            Td({
                ...
                if (linkTargets.isEmpty()) {
                    Span({ style { marginLeft(10.px); color(Color.gray); fontSize(12.px) } }) { Text("Crea primero el registro para poder vincularla.") }
                } else {
                    Span({ style { marginLeft(10.px); color(Color.gray); fontSize(12.px) } }) { Text("Vincular con:") }
                    Select({
                        style { marginLeft(4.px); padding(3.px); borderRadius(4.px); property("border", "1px solid #cbd5e1"); fontSize(12.px) }
                        onChange { selectedId = it.value?.toIntOrNull() ?: 0; linkMsg = "" }
                    }) {
                        Option("0") { Text("Selecciona un registro...") }
                        linkTargets.forEach { (id, label) -> Option(id.toString()) { Text(label) } }
                    }
                    Button({
                        ...
                        onClick {
                            if (selectedId <= 0) {
                                linkMsg = "Elige un registro"
                            } else {
                                scope.launch {
                                    try {
                                        val resp = client.post("$BACKEND_URL/api/v1/ehs/documentos/${doc.id}/vincular") {
                                            contentType(ContentType.Application.Json)
                                            setBody(EhsDocumentLinkRequest(moduleType = moduleType, moduleRecordId = selectedId))
                                        }
                                        if (resp.status.value in 200..299) onLinked()
                                        else linkMsg = "No se pudo vincular ese registro"
